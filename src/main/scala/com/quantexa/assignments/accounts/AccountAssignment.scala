...
//  accountDS.show
  ...
  //END GIVEN CODE
  val aggregatedAccountDF = accountDS
  ...
  val arrayAccountsDF = accountDS
    .groupByKey(_.customerId)
    .mapGroups((k, v) => (k, v.toArray))
    .toDF("customerId", "accounts")

  val customerAccountOutputDS = customerDS
    .join(aggregatedAccountDF, Seq("customerId"), "left_outer")
    .join(arrayAccountsDF, Seq("customerId"), "left_outer")
