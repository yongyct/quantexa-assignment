...
object AddressAssignment extends App{
  ...
  @tailrec
  def groupDataByStay(list: List[AddressData], currentGroup: AddressGroupedData,
                      groupedData: List[AddressGroupedData]): List[AddressGroupedData] = {
    list match {
      ...
      case x :: xs =>
        ...
        // Else add data to existing list
          ...
          val newCurrentGroup = currentGroup
            .copy(
              endDate = math.max(x.toDate, currentGroup.endDate),
              customerIds = currentGroup.customerIds :+ x.customerId
            )
  ...
  val addressGroups = groupDataByStay(sortedList, initialGroup, List[AddressGroupedData]())
    .sortBy(stay => (stay.addressId, stay.startDate))

  // Get number of groups found.
  val numberOfGroups = addressGroups.length

  addressGroups.foreach{println}
